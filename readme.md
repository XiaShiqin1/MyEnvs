...
2. **Tmux** (`prepare_tmux.sh`)：自动同步全局剪贴板与 Vim 习惯键位。
3. **Zsh + p10k** (`prepare_zsh.sh`)：增量挂载 `.zshrc`，全自动安装 MesloLGS NF 字体，自动配置 Terminal 渲染并恢复绝美 p10k 终端外观。
4. **Neovim** (`prepare_nvim.sh`)：安全备份本机配置，并同步你熟悉的 vim 环境。
